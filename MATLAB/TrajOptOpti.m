%% Attitude Trajectory Optimization
% Arthur Binstein and Marco Hinojosa
close all; clear all;
load EigenStates.mat
% qOpt = q;
% tauOpt = u;
% omOpt = w;

J = D;
N = size(qOpt,2);
Nx = N*11 - 4;
Nc = 7*(N-1)+N-2;

x0 = [qOpt; omOpt; tauOpt; dt*ones(1,N)];
x0 = x0(:);
% Initial Guess
x0(end-3:end) = [];
% Bounds on the states
lb = [-Inf*ones(4,N); -100*ones(3,N); -1500*ones(3,N); dt/10*ones(1,N)];
ub = [Inf*ones(4,N); 100*ones(3,N); 1500*ones(3,N); dt*1.5*ones(1,N)];
% lb = [-Inf*ones(4,N); -100*ones(3,N); -1500*ones(2,N); zeros(1,N); dt/10*ones(1,N)];
% ub = [Inf*ones(4,N); 100*ones(3,N); 1500*ones(2,N); zeros(1,N); dt*5*ones(1,N)];

lb = lb(:);
ub = ub(:);
lb(end-3:end) = [];
ub(end-3:end) = [];
% State Equality Constraints
Aeq = zeros(N-1,11*N-4);
for ii = 1:N-2
    Aeq(ii,11*ii) = 1;
    Aeq(ii,11*ii+11) = -1;
end
Aeq = [eye(7), zeros(7,11*(N-1)); zeros(7,11*(N-1)) eye(7); Aeq];
beq = zeros(N-1,1);
beq = [x0(1:7); qOpt(:,end); omOpt(:,end) ; beq];

cl = zeros(Nc,1);
cu = zeros(Nc,1);
for ii = 8:8:Nc
    cl(ii) = 1;
    cu(ii) = 1;
end
    

objGrad = zeros(1,11*N-4);
for ii = 1:length(objGrad)
    if mod(ii,11) == 0
        objGrad(ii) = 1;
    end
end

% Options
opts = optiset('solver','ipopt','display','iter');
% Build OPTI Problem
Opt = opti('fun',@(x) fun(x,objGrad),'grad',@(x) grad(x,objGrad),'nl',@(x) nl(x,N,J,q0,qd), ...
               cl,cu,'jac',@(x) jac(x,N,J,q0,qd),'bounds',lb,ub,'x0',x0, ...
               'eq',Aeq,beq,'options',opts);
           
[x,fval,exitflag,info] = solve(Opt);     

xmod = reshape(x(1:11*(N-1)),11,N-1);
qs = [xmod(1:4,:) x(end-6:end-3)];
ws = [xmod(5:7,:) x(end-2:end)];
us = xmod(8:10,:);
dts = xmod(11,:);
ts = 0:dts(1):sum(dts)+.001;
tOpt = 0:dt:(N-1)*dt;


subplot(4,1,1)
title('Quaternion')
hold on;
plot(tOpt,qOpt(1,:),'LineWidth',2)
plot(ts,qs(1,:),'LineWidth',2)
legend('Eigen-Axis Slew','Time-Optimal Slew')
subplot(4,1,2)
hold on;
plot(tOpt,qOpt(2,:),'LineWidth',2)
plot(ts,qs(2,:),'LineWidth',2)
subplot(4,1,3)
hold on;
plot(tOpt,qOpt(3,:),'LineWidth',2)
plot(ts,qs(3,:),'LineWidth',2)
subplot(4,1,4)
hold on;
plot(tOpt,qOpt(4,:),'LineWidth',2)
plot(ts,qs(4,:),'LineWidth',2)
xlabel('Time (s)')

figure;
subplot(3,1,1)
title('Omega')
hold on;
plot(tOpt,omOpt(1,:),'LineWidth',2)
plot(t,w(1,:),'LineWidth',2)
plot(ts,ws(1,:),'LineWidth',2)
legend('Eigen-Axis Slew','Bang-Bang Slew','Time-Optimal Slew')
subplot(3,1,2)
hold on;
plot(tOpt,omOpt(2,:),'LineWidth',2)
plot(t,w(2,:),'LineWidth',2)
plot(ts,ws(2,:),'LineWidth',2)
ylabel('Angular Velocity (rad/s)')
subplot(3,1,3)
hold on;
plot(tOpt,omOpt(3,:),'LineWidth',2)
plot(t,w(3,:),'LineWidth',2)
plot(ts,ws(3,:),'LineWidth',2)
xlabel('Time (s)')

figure;
subplot(3,1,1)
title('Control')
hold on;
plot(tOpt,tauOpt(1,:),'LineWidth',2)
plot(t,u(1,:),'LineWidth',2)
plot(ts(1:end-1),us(1,:),'LineWidth',2)
legend('Eigen-Axis Slew','Bang-Bang Slew','Time-Optimal Slew')
subplot(3,1,2)
hold on;
plot(tOpt,tauOpt(2,:),'LineWidth',2)
plot(t,u(2,:),'LineWidth',2)
plot(ts(1:end-1),us(2,:),'LineWidth',2)
ylabel('Torque (N-m)')
subplot(3,1,3)
hold on;
plot(tOpt,tauOpt(3,:),'LineWidth',2)
plot(t,u(3,:),'LineWidth',2)
plot(ts(1:end-1),us(3,:),'LineWidth',2)
xlabel('Time (s)')


function obj = fun(x,objGrad)
obj = objGrad*x;
end

function objGrad = grad(x,objGrad)
objGrad = objGrad;
end

function [ceq] = nl(x,N,J,q0,qd)
[ceq, ~] = nonlcon(x,N,J,q0,qd);
end

function [dceq] = jac(x,N,J,q0,qd)
[~, dceq] = nonlcon(x,N,J,q0,qd);
end

function [ceq, dceq] = nonlcon(x,N,J,q0,qd)
Nc = 7*(N-1)+N-2;
xmod = reshape(x(1:11*(N-1)),11,N-1);
q = [xmod(1:4,:) x(end-6:end-3)];
w = [xmod(5:7,:) x(end-2:end)];
u = xmod(8:10,:);
dt = xmod(11,:);

ceq = zeros(Nc,1);
dceq = zeros(Nc,11*N-4);
% size(dceq)
for ii = 1:N-1
    ii;
    qm = .5*(q(:,ii) + q(:,ii+1));
    wm = .5*(w(:,ii) + w(:,ii+1));
    [fx, dfx, du] = dynamics(qm,wm,u(:,ii),J);
    ceq(1+8*(ii-1):7+8*(ii-1),1) = [q(:,ii); w(:,ii)] + fx*dt(ii) - [q(:,ii+1); w(:,ii+1)];
    if ii ~= (N-1)
        ceq(8*ii,1) = q(:,ii)'*q(:,ii);
    end
    size(ceq);
    A = [];
    A = fillrows(fx, dfx, du, dt(ii), q(:,ii), ii, length(x));
    size(A);
    size(ceq);
    size(dceq);
    if ii ~= (N-1)
        dceq(1+8*(ii-1):8+8*(ii-1),:) =  A;
    else
        dceq(1+8*(ii-1):7+8*(ii-1),:) =  A;
    end
end
% dceq = dceq';
end

function [fx,dfx,du] = dynamics(q,w,u,J)
% function [fx] = dynamics(q,w,u,J)

fx(1:4,1) = .5*qmult(q)*[w;0];
fx(5:7,1) = -inv(J)*(cross(w,J*w) - u);

dfx = [.5*qmultR([w;0]) .5*qmult(q)*[eye(3); zeros(1,3)];
       zeros(3,4)       -inv(J)*(hat(w)*J - hat(J*w))];

du = [zeros(4,3); inv(J)];

end

function A = fillrows(fx, dfx, du, dt, q, n, N)
m = n-1;

if n ~= (N+4)/11 - 1
    A = zeros(8,N);
    A(1:7,m*11+1:m*11+7) = eye(7) + dfx*.5*eye(7)*dt;
    A(1:7,m*11+8:m*11+10) = dt*du;
    A(1:7,m*11+11) = fx;
    A(1:7,m*11+12:m*11+18) = dfx*.5*eye(7)*dt - eye(7);
    A(8,m*11+1:m*11+4) = 2*q';  
else
    A = zeros(7,N);
    A(1:7,m*11+1:m*11+7) = eye(7) + dfx*.5*eye(7)*dt;
    A(1:7,m*11+8:m*11+10) = dt*du;
    A(1:7,m*11+11) = fx;
    A(1:7,m*11+12:m*11+18) = dfx*.5*eye(7)*dt - eye(7);
end

end

